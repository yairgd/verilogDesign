...
class Section extends verilog.awt.ShapeList {
	...
	Line2D line2d = new Line2D.Float(P1.getPoint(), P2.getPoint());
	 float[] dash2 = { 1f,1f };
	    BasicStroke bs2 = new BasicStroke(width);//  (2, BasicStroke.JOIN_ROUND, BasicStroke.JOIN_ROUND, 1.0f, dash2,15f);
	...
	if (Math.sqrt((x - x0) * (x - x0) + (y - y0) * (y - y0)) < 3  &&
		y0>Math.min(P1.getPoint().y, P2.getPoint().y) &&
		y0<Math.max(P1.getPoint().y, P2.getPoint().y) ) {
...
public class Line extends verilog.awt.ShapeList {
    ...
    /*Line(ConectionPoint p1,ConectionPoint p2) {
   	// sectionList.add(new Section(null,null));
   	shapes.add(new Section(null, null));
       }*/
	...
	for (final Shape shape : shapes) {
	    ...
	    if (section.getP1().getShapeStatus() == ShapeStatus.Move
		    || section.getP2().getShapeStatus() == ShapeStatus.Move
		    || section.getShapeStatus() == ShapeStatus.Move) {
		curSection = section;
