...
public class Model extends JPanel implements MouseListener,MouseMotionListener {
	...
	for (Shape shape : drawingPanel.shapeList.shapes) {
		...
		if (area.width < polygon.getMinX()) {
		    area.width = polygon.getMinX()
			    + ((Block) shape).getSize().x + 100;
		    change = true;
		}
		if (area.height < polygon.getMinY()) {
		    area.height = polygon.getMinY()
			    + ((Block) shape).getSize().y + 100;
		    change = true;
		}
	    }
	}
	...
	/*System.out.print("bbbbbbbbbbbbbbbb " + area.width + "  " + area.height
		+ "\n");*/
...
class DrawingPane1 extends JScrollPane implements MouseListener,
	...
	if (e.isPopupTrigger()) {
		...
		if (shape instanceof Polygon) {
		    JPopupMenu pm = ((Polygon) shape).getPopupMenu();
		    if (pm != null
			    && ((Polygon) shape).isInside(event.getPoint())) {
			pm.show(e.getComponent(), e.getX(), e.getY());
			b = true;
	...
	if (command.equals(verilogGui.getString("new_block"))) {
	    ...
	    block.setLocaion(new Point(this.getMousePosition().x, this
		    .getMousePosition().y));
